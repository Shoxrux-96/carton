...
export function buildDeliveryMap(wLat: number, wLng: number): string {
  ...
  return '<!DOCTYPE html><html><head>'
    ...
    + leafSc
    + '<style>*{margin:0;padding:0}html,body,#map{width:100%;height:100%}.ct{position:fixed;top:10px;right:10px;z-index:9999;display:flex;flex-direction:column;gap:5px}.bt{width:38px;height:38px;border-radius:10px;background:#fff;border:none;box-shadow:0 2px 8px rgba(0,0,0,0.15);display:flex;align-items:center;justify-content:center;font-size:15px;cursor:pointer}.bt.on{background:#f97316}.myLoc{position:fixed;bottom:60px;right:10px;z-index:9999;width:44px;height:44px;border-radius:22px;background:#3b82f6;border:3px solid white;box-shadow:0 2px 12px rgba(59,130,246,0.5);display:flex;align-items:center;justify-content:center;font-size:18px;cursor:pointer}.myLoc:active{transform:scale(0.9)}.dst{position:fixed;bottom:8px;left:8px;right:8px;background:rgba(255,255,255,0.95);border-radius:14px;padding:10px 14px;box-shadow:0 2px 12px rgba(0,0,0,0.1);font-family:-apple-system,system-ui;font-size:12px;display:none;z-index:9999}.dst.show{display:block}.dst b{font-size:14px;display:block;margin-bottom:3px}.dst .r{display:flex;justify-content:space-between;margin-top:3px;color:#57534e}.dst .r span{font-weight:700;color:#1c1917}.pu{animation:p 1.5s infinite}@keyframes p{0%,100%{transform:scale(1)}50%{transform:scale(1.2);opacity:.7}}.myDot{width:18px;height:18px;background:#3b82f6;border:3px solid white;border-radius:50%;box-shadow:0 0 0 2px #3b82f6,0 2px 8px rgba(59,130,246,0.4);}</style>'
    ...
    + '<button class="myLoc" id="myLocBtn" onclick="getMyLoc()">&#x1F4CD;</button>'
    ...
    // Joriy lokatsiya marker
    + 'var myMarker=null;var myAccuracy=null;'
    + 'function getMyLoc(){if(!navigator.geolocation){alert("Geolocation qo\u0027llab-quvvatlanmaydi");return;}navigator.geolocation.getCurrentPosition(function(p){var lat=p.coords.latitude;var lng=p.coords.longitude;var acc=p.coords.accuracy;if(myMarker){m.removeLayer(myMarker);}if(myAccuracy){m.removeLayer(myAccuracy);}myMarker=L.marker([lat,lng],{icon:L.divIcon({className:"",html:\'<div class="myDot"></div>\',iconSize:[18,18],iconAnchor:[9,9]})}).addTo(m).bindPopup("\u041C\u0435\u043D\u0438\u043D\u0433 \u043B\u043E\u043A\u0430\u0442\u0441\u0438\u044F\u043C");myAccuracy=L.circle([lat,lng],{radius:acc,color:"#3b82f6",fillOpacity:0.1,weight:1}).addTo(m);m.flyTo([lat,lng],16,{duration:1.5});},function(e){alert("Lokatsiya olishda xatolik: "+e.message);},{enableHighAccuracy:true,timeout:10000});}'
    // Avtomatik lokatsiya olish
    + 'if(navigator.geolocation){navigator.geolocation.getCurrentPosition(function(p){var lat=p.coords.latitude;var lng=p.coords.longitude;if(!myMarker){myMarker=L.marker([lat,lng],{icon:L.divIcon({className:"",html:\'<div class="myDot"></div>\',iconSize:[18,18],iconAnchor:[9,9]})}).addTo(m).bindPopup("\u041C\u0435\u043D\u0438\u043D\u0433 \u043B\u043E\u043A\u0430\u0442\u0441\u0438\u044F\u043C");}},{timeout:8000});}'
