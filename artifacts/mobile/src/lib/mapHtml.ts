...
export function buildDeliveryMap(wLat: number, wLng: number): string {
  ...
  return '<!DOCTYPE html><html><head>'
    ...
    // Joriy lokatsiya — doimiy kuzatish
    + 'var myMarker=null;var myAccuracy=null;var myWatchId=null;var firstLoc=true;'
    + 'function updateMyLoc(p){var lat=p.coords.latitude;var lng=p.coords.longitude;var acc=p.coords.accuracy;if(myMarker){myMarker.setLatLng([lat,lng]);}else{myMarker=L.marker([lat,lng],{icon:L.divIcon({className:"",html:\'<div class="myDot"></div>\',iconSize:[18,18],iconAnchor:[9,9]})}).addTo(m).bindPopup("\u041C\u0435\u043D\u0438\u043D\u0433 \u043B\u043E\u043A\u0430\u0442\u0441\u0438\u044F\u043C");}if(myAccuracy){myAccuracy.setLatLng([lat,lng]).setRadius(acc);}else{myAccuracy=L.circle([lat,lng],{radius:acc,color:"#3b82f6",fillOpacity:0.1,weight:1}).addTo(m);}if(firstLoc){firstLoc=false;m.setView([lat,lng],15);}}'
    + 'function getMyLoc(){if(!navigator.geolocation){return;}navigator.geolocation.getCurrentPosition(function(p){updateMyLoc(p);},function(){},{enableHighAccuracy:true,timeout:10000,maximumAge:3000});}'
    + 'if(navigator.geolocation){myWatchId=navigator.geolocation.watchPosition(function(p){updateMyLoc(p);},function(){},{enableHighAccuracy:true,timeout:15000,maximumAge:5000});}'
